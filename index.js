...
const TobzKey = 'h0Mta0ZwZ95vHlxTDq8p'
...
• Limit: ${limitawal-updated.limit}`
                ...
                break
					...
					if (!isRegistered) return reply(ind.noregis())
					if (isLimit(sender)) return reply(ind.limitend(pusname))
					if (!q.includes('/')) return  reply(`*Cara Main*\n\n${prefix}dadu angka yang mau dilempar / jumlah taruhan\n*CONTOH*\n${prefix}dadu 5 / 5000`)
                			const dadu = q.substring(0, q.indexOf('/') - 1)
                			const taruh = q.substring(q.lastIndexOf('/') + 1)
                			if(isNaN(taruh)) return await reply('Lu mau taruhan brp?')
					if ( dadu > 12 ) return await reply( 'Batas Dadu Sampai 12' )
					confirmATM(sender, taruh);
					var hape = '';
					setTimeout( () => {
					const lemBot = Math.floor(Math.random() * 12)
					const randRew = Math.ceil(Math.random() * 1500)
					const rew = taruh + randRew
					
					if ( dadu < lemBot ) {
					hape = '*Kamu Kalah!!*';
					} else if ( dadu > lemBot ) {
					hape = '*Kamu Menang!!*'
					addKoinUser(sender, rew)
					} else {
					hape = '*SERI*'
					}
					...
					setTimeout( () => {
						reply('*Wait Ganz*, sedang bermain dengan Mastah pro pler...')
					}, 0); //1 sec
					await limitAdd(sender)
					break
...
*/
			...
			if (!isNsfw) return reply(ind.nsfwoff())
				...
				if (isLimit(sender)) return reply(ind.limitend(pusname))
					reply(ind.wait())
					asw = await fetchJson(`https://onlydevcity.herokuapp.com/api/nsfw/eroyuri?apikey=${devcityapi}`)
					pler = await getBuffer(asw.result.url)
					client.sendMessage(from, pler, image, {quoted: mek})
					await limitAdd(sender)
					break
...
*Last Update*:
_24 - 03 - 2021 17:50:55_
stickermeme
epepserti 
isnstavid
eroyuri

_25 - 03 2021 13:30:20_
casino (Kebanyakan kalah)
dadu (Jir error)
...
╠☞ *${prefix}lovemsg* [text]
╠☞ *${prefix}smoke* [text]
╠☞ *${prefix}crossgun* [text]
╠☞ *${prefix}logoepep* [text]
╠☞ *${prefix}epepsrti* [nama]
...
/*<==========================================[MUSIC&MEDIA MENU]==============================================>*/
		...
		break 
				...
				if (!isRegistered) return reply(ind.noregis())
                   		if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (!isUrl(args[0]) && !args[0].includes('www.instagram.com')) return reply(ind.error.lv)
				    anu = await fetchJson(`https://onlydevcity.herokuapp.com/api/ig?url=${args[0]}&apikey=${devcityapi}`, {method: 'get'})
				    insta = getBuffer(anu.hasil.result.data.data)
				    reply(ind.wait)
...
/*<==========================================[AKHIR MUSIC&MEDIA MENU]==============================================>*/
			...
			break
				...
				case 'stickmeme':
					...
					ranp = getRandom('.gif')
					rano = getRandom('.webp')
					peler = await fetchJson(`https://onlydevcity.herokuapp.com/api/stickermeme?teks=${pler}&img_url=${teks}&apikey=${devcityapi}`, {method: 'GET'})
					exec(`wget ${peler.result.image} -O ${ranp} && ffmpeg -i ${ranp} -vcodec libwebp -filter:v fps=fps=20 -lossless 1 -loop 0 -preset default -an -vsync 0 -s 512:512 ${rano}`, (err) => {
					fs.unlinkSync(ranp)
					buffer = fs.readFileSync(rano)
